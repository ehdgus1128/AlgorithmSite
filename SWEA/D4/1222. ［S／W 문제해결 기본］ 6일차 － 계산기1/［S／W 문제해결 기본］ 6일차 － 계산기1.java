...
//		System.out.println("convertMathExp str : "+str);
...
//			System.out.print(c + " ");
			if(c >= '0' && c <= '9') { // 숫자면 그냥 출력
				...
				char curValue; // stack 마지막 연산자				
				if(operatorArr.isEmpty()) { // stack 비어있으면 push
					operatorArr.push(c);
				} else {
					switch (c) {
					case '(' : // 1번째 우선순위
					...
					case '*' :
					case '/' : // 2번째 우선순위
						curValue = operatorArr.peek();
						if(curValue == '(') {
							operatorArr.push(c);
						}
						else convertStr.add(operatorArr.pop());			
						break;
					case '+' :
					case '-' : // 3번째 우선순위
						curValue = operatorArr.peek();
...
//				System.out.print(" size : "+operand.size()+" ");
				if(operand.size() >= 2) {
